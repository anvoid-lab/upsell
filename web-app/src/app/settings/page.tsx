...
export default function SettingsPage() {
  const { channels, aiSettings, isLoading, isSaving, saved, handleConnect, handleDisconnect, updateAISetting, handleSaveAI } = useSettings();
            ...
            <div className="bg-white border border-zinc-200 rounded-xl divide-y divide-zinc-100">
              ...
              <div className="p-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="icon" className="w-7 h-7 rounded-full" onClick={() => updateAISetting("followUpDelayHours", Math.max(1, aiSettings.followUpDelayHours - 1))}>−</Button>
                  <span className="w-8 text-center text-sm font-semibold text-zinc-900">{aiSettings.followUpDelayHours}h</span>
                  <Button variant="outline" size="icon" className="w-7 h-7 rounded-full" onClick={() => updateAISetting("followUpDelayHours", Math.min(48, aiSettings.followUpDelayHours + 1))}>+</Button>
                </div>
              </div>

              {/* Toggles */}
              {([
                ["useUrgency",    "Urgency",      "Alert when stock is low"],
                ["useUpsell",     "Upsell",       "Suggest related products"],
                ["useSocialProof","Social proof",  "Share customer reviews"],
                ["useCartRecovery","Cart recovery","Re-engage silent leads"],
    ...
    <div className="flex items-center justify-between p-4 bg-white border border-zinc-200 rounded-xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {channel.connected
            ? <p className="text-xs text-zinc-400">{channel.accountName} · Connected {channel.connectedAt}</p>
