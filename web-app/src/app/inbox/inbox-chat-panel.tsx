...
import {
  MoreHorizontal, Zap, Paperclip, Smile, Sparkles, PenLine,
  Check, CheckCheck, ChevronDown, FileText, UserPlus, Hash, Link, X,
} from "lucide-react";
...
import type { Message, AISuggestion, ConversationStatus, Platform } from "@/types";
...
const TEMPLATES = [
  { id: "t1", label: "Stock disponível",      text: "Olá! O produto está disponível em stock. Quer finalizar a compra?" },
  { id: "t2", label: "Envio confirmado",      text: "A sua encomenda foi enviada! Entrega estimada: 3–5 dias úteis." },
  { id: "t3", label: "Desconto especial",     text: "Temos uma oferta especial — 10% de desconto no próximo pedido. Código: SAVE10" },
  { id: "t4", label: "Follow-up pós-compra",  text: "Olá! Ficou satisfeito com a sua encomenda? Posso ajudar com mais alguma coisa?" },
  { id: "t5", label: "Produto esgotado",      text: "Lamentamos, o produto está temporariamente esgotado. Quer ser notificado quando voltar?" },
];
...
const STATUS_DOT: Record<ConversationStatus, string> = {
  open: "bg-indigo-500", pending: "bg-amber-500", resolved: "bg-emerald-500",
};
...
export const InboxChatPanel: FC<InboxChatPanelProps> = ({ selectedId, onStatusChange, onClose }) => {
  const {
    conversation, messages, suggestion, replyText, isLoading, isSending,
    suggestionStatus, setReplyText, handleSendReply, handleSendSuggestion,
    handleScheduleSuggestion, handleDismissSuggestion,
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`https://vendai.app/conversations/${selectedId}`);
  };

  const templateQuery   = replyText.startsWith("/") ? replyText.slice(1).toLowerCase() : "";
  const filteredTpls    = TEMPLATES.filter(
    t => !templateQuery || t.label.toLowerCase().includes(templateQuery) || t.text.toLowerCase().includes(templateQuery)
  );
  ...
  if (!selectedId) return (
    <div className="flex-1 flex items-center justify-center bg-zinc-50/50">
      <div className="text-center">
        ...
        <p className="text-sm font-semibold text-zinc-600">Select a conversation</p>
        <p className="text-xs text-zinc-400 mt-1">Choose from the list on the left</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="h-[52px] px-4 border-b border-zinc-200 flex items-center gap-2 flex-shrink-0 bg-white">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[140px]">
            {(["open", "pending", "resolved"] as ConversationStatus[]).map(s => (
              <DropdownMenuItem key={s} onClick={() => handleStatusChange(s)} className={cn("text-xs", convStatus === s && "font-semibold")}>
                <span className={cn("w-1.5 h-1.5 rounded-full mr-2 flex-shrink-0", STATUS_DOT[s])} />
                {STATUS_LABELS[s]}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium border border-zinc-200 text-zinc-500 hover:border-zinc-300 transition-colors">
              {assignedMember
                ? <><span className="w-4 h-4 rounded-full bg-indigo-100 text-indigo-600 text-[9px] font-bold flex items-center justify-center">{assignedMember.initials}</span>{assignedMember.name.split(" ")[0]}</>
          ...
          <DropdownMenuContent align="end" className="min-w-[160px]">
            <p className="px-2 py-1.5 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">Assign to</p>
            <DropdownMenuSeparator />
            {TEAM_MEMBERS.map(m => (
              <DropdownMenuItem key={m.id} onClick={() => setAssignedTo(m.id)} className={cn("text-xs", assignedTo === m.id && "font-semibold")}>
                <span className="w-5 h-5 rounded-full bg-zinc-100 text-zinc-600 text-[9px] font-bold flex items-center justify-center mr-2">{m.initials}</span>
                {m.name}
              </DropdownMenuItem>
            ))}
            {assignedTo && (
              <>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setAssignedTo(null)} className="text-xs text-zinc-400">Unassign</DropdownMenuItem>
              </>
            )}
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="w-7 h-7 rounded-full text-zinc-400">
              <MoreHorizontal className="w-3.5 h-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="min-w-[180px]">
            <DropdownMenuItem onClick={handleCopyLink} className="text-xs gap-2">
              <Link className="w-3.5 h-3.5" /> Copy conversation link
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => handleRequestClose()} className="text-xs text-red-600 gap-2 focus:text-red-600">
      ...
      {/* ── AI suggestion ── */}
      {suggestion && (suggestionStatus === "idle" || suggestionStatus === "sending") && (
        <div className="mx-4 mb-2 flex-shrink-0 relative">
          ...
          <div className="relative p-3.5 bg-white border border-indigo-200 rounded-2xl">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-semibold text-indigo-600 flex-1">AI follow-up suggestion</span>
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={() => handleSendSuggestion(editedSuggestion !== suggestion.message ? editedSuggestion : undefined)}
                disabled={suggestionStatus === "sending"}
                size="sm" className="rounded-full h-7 px-3 text-xs"
              >
                Send now
              </Button>
              <Button onClick={() => handleScheduleSuggestion(6)} variant="outline" size="sm" className="rounded-full h-7 px-3 text-xs">
                Schedule 6h
              </Button>
              <Button onClick={handleDismissSuggestion} variant="ghost" size="sm" className="rounded-full h-7 px-3 text-xs text-zinc-400">
                Dismiss
              </Button>
            </div>
          </div>
      ...
      <div className="mx-4 mb-4 flex-shrink-0">
        ...
        <div className={cn(
          ...
          <div className={cn(
            ...
            replyMode === "note" ? "border-amber-200 bg-amber-50/50" : "border-zinc-100"
          )}>
            {replyMode === "reply" ? (
              <>
                <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full", PLATFORM_COLORS[contact.platform])}>
                  {PLATFORM_LABELS[contact.platform]}
                </span>
                <span className="text-[10px] text-zinc-400">→ {contact.name}</span>
          ...
          {showTemplates && filteredTpls.length > 0 && (
            <div className="border-b border-zinc-100">
              ...
              {filteredTpls.map(t => (
                <button
                  key={t.id}
                  onMouseDown={e => { e.preventDefault(); selectTemplate(t.text); }}
                  className="w-full flex flex-col px-3 py-2 text-left hover:bg-zinc-50 border-t border-zinc-50 transition-colors"
                >
                  <span className="text-xs font-medium text-zinc-800">{t.label}</span>
                  <span className="text-[11px] text-zinc-400 truncate">{t.text}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {showEmojiPicker && (
            <div className="border-b border-zinc-100 px-3 py-2 flex flex-wrap gap-1">
              {EMOJIS.map(e => (
                <button
                  key={e}
                  onMouseDown={ev => { ev.preventDefault(); handleEmojiSelect(e); }}
          ...
          <Textarea
            ...
            placeholder={replyMode === "reply" ? "Type a message… (⌘↵ to send)" : "Write an internal note…"}
            rows={2}
            className={cn(
              "border-0 shadow-none rounded-none focus-visible:ring-0 resize-none text-sm px-3 py-2",
              replyMode === "note" ? "bg-amber-50/30 placeholder:text-amber-300" : "placeholder:text-zinc-300"
            )}
          />
          ...
          <div className={cn(
            ...
            replyMode === "note" ? "border-amber-200 bg-amber-50/30" : "border-zinc-100"
          )}>
            <div className="flex gap-0.5">
              <Button variant="ghost" size="icon" className="w-7 h-7 rounded-full text-zinc-400" title="Templates" onClick={openTemplates}>
                <Zap className="w-3.5 h-3.5" />
              </Button>
              <Button variant="ghost" size="icon" className="w-7 h-7 rounded-full text-zinc-400" title="Attach file" onClick={() => fileInputRef.current?.click()}>
                <Paperclip className="w-3.5 h-3.5" />
              </Button>
              <Button
                variant="ghost" size="icon"
                className={cn("w-7 h-7 rounded-full", showEmojiPicker ? "text-indigo-500 bg-indigo-50" : "text-zinc-400")}
                title="Emoji"
            ...
            <div className="flex items-center gap-1.5">
              <div className="flex items-center gap-0.5 p-0.5 bg-zinc-100 rounded-full mr-1">
                <button
                  ...
                  className={cn(
                    ...
                    replyMode === "reply" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-400 hover:text-zinc-600"
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium transition-all",
                    replyMode === "note" ? "bg-amber-500 text-white shadow-sm" : "text-zinc-400 hover:text-zinc-600"
                  )}
                >
              ...
              <Button
                ...
                className={cn(
                  ...
                  replyMode === "note" && "bg-amber-500 hover:bg-amber-600 text-white"
                )}
              >
                {isSending ? "Sending…" : replyMode === "note" ? "Add note" : "Send"}
              </Button>
            </div>
          </div>
        </div>
...
const MessageBubble: FC<{ message: Message }> = ({ message }) => {
  ...
  return (
    <div className={cn("flex max-w-[72%]", isOut ? "self-end flex-col items-end" : "self-start flex-col items-start")}>
      <div className={cn(
